...
def create_account():
    ...
    if old_account:
        error_msg(f"Error: Account with email {email} alredy exists.")
        return

    state.active_account = svc.create_account(name, email)
    success_msg(f"Created new account id {state.active_account.id}")
...
def log_into_account():
    ...
    success_msg('Logged succesfully.')
...
def register_cage():
    ...
    allow_dangerous = input('Can you host venemous snakes [y, n]?').lower().startswith('y')
    name = input('Give your cage a name: ')
    price = input('How much cost?')

    cage = svc.register_cage(state.active_account, name, allow_dangerous, has_toys,
                             carpeted, meters, price)
...
def view_bookings():
    ...
    if not state.active_account:
        error_msg("You must log in first to view your bookings.")
        return
