...
def add_available_date(cage, start_date: datetime, days: int) -> Cage:
    booking = Booking()
...
def add_snake(account, name, length, specie, is_venemous) -> Snake:
    snake = Snake()
    snake.name = name
    snake.length = length
    snake.species = specie
...
def get_available_cages(checkin: datetime,
                        checkout: datetime, snake: Snake) -> List[Cage]:
...
def book_cage(account, snake: Snake, cage: Cage, checkin, checkout):
    booking = None
